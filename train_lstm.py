...
# Convert full_date to datetime BEFORE baseline computation
...
baseline['personal_std'] = baseline['personal_std'].fillna(1).clip(lower=1)  # avoid div by 0
df = df.merge(baseline, on='user_id', how='left')

# Normalise: how far from this person's normal?
df['score_vs_baseline'] = (
    (df['productivity_score'] - df['personal_mean'])
    / df['personal_std']
)

# Normalised user ID so model knows WHO it's predicting for
...
print(f"Personal baseline computed for {len(baseline)} employees")
print(f"Baseline saved → models/baseline.pkl")
...
# ════════════════════════════════════════════════════════════
# 3. FEATURES LIST — 20 features total (18 + 2 personal context)
# ════════════════════════════════════════════════════════════
FEATURES = [
    # Personal context (2) — FIX 2: individual prediction
    'user_id_norm',
    'score_vs_baseline',
    # Core attendance (4)
    'hours_worked',
