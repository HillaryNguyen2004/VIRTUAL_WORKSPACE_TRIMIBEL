@props(['title' => '', 'subtitle' => '', 'btnLabel' => '', 'hrefAll' => '', 'hrefAction' => '', 'animationDelay' => '', 'bgBtn' => '', 'bgHoverBtn' => ''])

<div
    class="flex flex-col justify-between gap-5 w-full h-full bg-[#FDFDFF] shadow-[0_4px_40px_0_rgba(32,27,53,0.1)] rounded-[20px] py-5 px-6 animate-fade-in-up {{ $animationDelay }}">
    ...
    <a href="{{ $hrefAction }}"
        class="flex items-center justify-center py-2 px-3 border rounded-xl text-white {{ $bgBtn }} hover:{{ $bgHoverBtn }} shadow-xl transition">{{ $btnLabel }}</a>
</div>