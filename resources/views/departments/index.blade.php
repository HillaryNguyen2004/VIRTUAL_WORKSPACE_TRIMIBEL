                            ...
                            <tr>
                                <td class="py-4 pr-4">
                                    <form method="POST" action="{{ route('admin.departments.update', $department) }}" class="flex items-center gap-2">
                                        @csrf
                                        @method('PUT')
                                        <input name="name" type="text" value="{{ $department->name }}" class="w-full bg-canvas border text-main h-[42px] px-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" required>
