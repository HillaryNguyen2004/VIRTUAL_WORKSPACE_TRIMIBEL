...
@section('content')
    @vite(['resources/utils/validate_pwd_reset.js'])
    ...
    <div class="h-full w-full">
        ...
        <div class="flex flex-col items-center justify-center w-full h-full pb-10 m-auto">
            <h1 class="text-xl md:text-3xl font-medium text-gray-900 text-center">{{ __('reset_password.title') }}</h1>
            <p class="text-sm md:text-base my-2 text-center text-gray-500 w-full sm:w-96">
                {{ __('reset_password.subtitle') }}
            </p>

            
            <form method="POST" action="{{ route('password.update') }}" class="space-y-6 w-full sm:w-96">
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">

                <div class="mb-3 hidden">
                    <input type="email" name="email" placeholder="{{ __('reset_password.email') }}" value="{{ $email ?? old('email') }}" required 
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-800">{{ __('reset_password.new_password') }}</label>
                    <div class="relative">
                        <input id="password" name="password" type="password" placeholder="{{ __('reset_password.password_placeholder') }}" autofocus class="text-sm md:text-base mt-1 block w-full rounded-xl border border-gray-300 px-4 py-3 pr-12
                        ...
                        <button type="button" id="togglePwd" class="absolute inset-y-0 right-2 p-3"
                            aria-label="{{ __('reset_password.show_password_aria') }}" aria-controls="password" aria-pressed="false">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-800">
                        {{ __('reset_password.confirm_password_label') }}
                    </label>
                    <div class="relative">
                        <input id="password_confirmation" name="password_confirmation" type="password"
                            placeholder="{{ __('reset_password.confirm_password_placeholder') }}" class="text-sm md:text-base mt-1 block w-full rounded-xl border border-gray-300 px-4 py-3 pr-12
                                        placeholder-gray-400 hover:border-gray-400 focus:outline-none
                                        focus:border-[#5D3FD3] transition text-[#5D3FD3]" />
                    ...
                    <ul id="validate-pwd" class="mt-3 ml-2 space-y-1 text-[12px]">
                        <li id="at-least-8-words" class="text-gray-300 flex items-center gap-2">
                                ...
                                {{ __('reset_password.validation_8_chars') }}
                            </li>
                            <li id="at-least-1-spe-char" class="text-gray-300 flex items-center gap-2">
                                ...
                                {{ __('reset_password.validation_1_special') }}
                            </li>
                            <li id="at-least-1-number" class="text-gray-300 flex items-center gap-2">
                                ...
                                {{ __('reset_password.validation_1_number') }}
                            </li>
                    </ul>
                ...
                <button type="submit" class="btn btn-primary w-full py-2 px-3 rounded-xl text-white font-medium bg-btn-login
                    ...
                    <span data-label>{{ __('reset_password.submit_button') }}</span>
                </button>
            </form>
            <div class="flex gap-2 text-center items-center text-xs md:text-sm mt-4 text-gray-500 hover:underline">
                ...
                <a href="{{ route('login') }}">{{ __('reset_password.back_to_login') }}</a>
            </div>
        </div>
    </div>
@endsection