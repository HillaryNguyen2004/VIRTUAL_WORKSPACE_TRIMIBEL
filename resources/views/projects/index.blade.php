    ...
    <div class="flex flex-col gap-6 w-full w-max-[1200px] mx-auto text-main px-4 md:px-8 lg:px-16 xl:px-24 py-8">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center w-full mb-8">
            ...
            <a href="{{ route('projects.create') }}"
                ...
                <span class="font-medium">{{ __('projects.create_project') }}</span>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-muted-200 shadow-lg shadow-main/5 overflow-hidden flex flex-col animate-fade-in-up">
            ...
            {{-- FILTERS --}}
            <form method="GET" class="p-5 border-b border-muted-200 flex flex-wrap gap-4 bg-white">
                ...
                <select name="sort_dir" class="bg-canvas border border-muted-200 text-main py-2.5 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer hover:border-primary/50">
            ...
            <div class="overflow-x-auto w-full">
                <table class="w-full table-fixed">
                    <thead class="bg-muted-50 border-b border-muted-200">
                        <tr>
                            <th class="w-[25%] py-4 pl-6 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">{{ __('projects.title') }}</th>
                            <th class="w-[20%] py-4 px-3 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">{{ __('projects.staff') }}</th>
                            <th class="w-[20%] py-4 px-3 text-center text-xs font-semibold text-muted-400 uppercase tracking-wider">{{ __('projects.status') }}</th>
                            <th class="w-[15%] py-4 px-3 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">{{ __('projects.start_date') }}</th>
                            <th class="w-[15%] py-4 px-3 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">{{ __('projects.due_date') }}</th>
                        ...
                        @forelse($projects as $project)
                            ...
                            <tr class="hover:bg-canvas transition-colors">
                                ...
                                <td class="py-4 px-3 text-sm text-muted-500">
                                    {{ $project->staff->name ?? 'N/A' }}
                                </td>
                                <td class="flex justify-center py-4 px-3">
                                ...
                                <td class="py-4 pr-6 text-right">
                                    <div class="flex justify-end gap-1">
                                        <a href="{{ route('projects.edit', $project->id) }}"
                                            class="p-1.5 rounded-lg text-muted-400 hover:bg-secondary/10 hover:text-secondary transition-colors" title="{{ __('projects.edit') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-4 h-4 fill-current">
                                        ...
                                        <form action="{{ route('projects.destroy', $project->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button class="p-1.5 rounded-lg text-muted-400 hover:bg-danger/10 hover:text-danger transition-colors" title="{{ __('projects.delete') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-4 h-4 fill-current">
