...
@section('content')
    ...
    <div class="flex flex-col gap-6 w-full w-max-[1200px] mx-auto text-main px-4 md:px-8 lg:px-16 xl:px-24 py-8">
        ...
        <div class="w-full bg-white rounded-2xl p-6 border border-muted-200 shadow-lg shadow-main/5 relative overflow-hidden animate-fade-in-up">
            ...
            <form action="{{ route('projects.update', $project->id) }}" method="POST" class="relative z-10">
                @csrf
                @method('PUT')

                {{-- Reusable Classes (Copied from Create Theme) --}}
                @php
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="col-span-2"> 
                        ...
                        <textarea name="description" class="rich-text {{ $inputClass }} h-[200px] resize-none" placeholder="{{ __('projects.description_placeholder') }}" required>{{ old('description', $project->description) }}</textarea> 
                    </div>
                </div>
