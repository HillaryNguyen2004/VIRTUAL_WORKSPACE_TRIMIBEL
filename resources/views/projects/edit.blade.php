            ...
            <form action="{{ route('projects.update', $project->id) }}" method="POST" class="relative z-10">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" class="{{ $inputClass }} min-h-[120px] resize-none">{{ old('description', $project->description) }}</textarea>
                    </div>
                            ...
                            <div class="relative">
                                <select name="staff_id" class="{{ $inputClass }} appearance-none" required>
                                    @foreach($staffUsers as $staff)
                                        <option value="{{ $staff->id }}" {{ (old('staff_id', $project->staff_id) == $staff->id) ? 'selected' : '' }}>
                                            {{ $staff->name }}
                                        </option>
                                    @endforeach
                                </select>
