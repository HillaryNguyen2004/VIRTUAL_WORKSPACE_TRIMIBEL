...
@section('content')
    ...
    <div class="flex flex-col gap-6 w-full w-max-[1200px] mx-auto text-main px-4 md:px-8 lg:px-16 xl:px-24 py-8">
        ...
        <div class="bg-white rounded-2xl border border-muted-200 shadow-lg shadow-main/5 overflow-hidden flex flex-col animate-fade-in-up">
            ...
            <form class="p-5 border-b border-muted-200 flex flex-wrap gap-4 bg-white" method="GET" action="{{ url()->current() }}">
                <div class="flex-1 min-w-[200px] relative">
                    ...
                    <input type="text" name="search" id="search" placeholder="{{ __('tasks.search_placeholder') }}" value="{{ request('search') }}"
                ...
                <select name="sort_dir" id="sort_dir"
                    ...
                    <option value="asc" {{ request('sort_dir') === 'asc' ? 'selected' : '' }}>{{ __('tasks.name_asc') }}</option>
                    <option value="desc" {{ request('sort_dir') === 'desc' ? 'selected' : '' }}>{{ __('tasks.name_desc') }}</option>
