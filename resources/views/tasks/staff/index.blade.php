    ...
    <x-action-layout :route="$dashRoute" :title="'profile.back_to_dashboard'">
        ...
        <form class="flex flex-wrap gap-2 animate-fade-in-up [animation-delay:150ms]" method="GET">
            ...
            <select name="status" id="status"
                ...
                <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>{{ __('tasks.pending') }}
                </option>
                <option value="in_progress" {{ request('status') == 'in_progress' ? 'selected' : '' }}>
                    {{ __('tasks.in_progress') }}
                </option>
                <option value="completed" {{ request('status') == 'completed' ? 'selected' : '' }}>{{ __('tasks.completed') }}
                </option>
            </select>
            <select name="sort" id="sort"
                class="rounded-xl text-sm md:text-base border border-gray-300 px-3 py-2 placeholder-gray-400 hover:border-gray-400 focus:outline-none focus:border-[#5D3FD3] transition cursor-pointer">
                <option value="">{{ __('tasks.default_sort') }}</option>
                <option value="name_asc" {{ request('sort') == 'name_asc' ? 'selected' : '' }}>{{ __('tasks.name_asc') }}
                </option>
                <option value="name_desc" {{ request('sort') == 'name_desc' ? 'selected' : '' }}>{{ __('tasks.name_desc') }}
                </option>
                <option value="due_asc" {{ request('sort') == 'due_asc' ? 'selected' : '' }}>{{ __('tasks.due_asc') }}
                </option>
                <option value="due_desc" {{ request('sort') == 'due_desc' ? 'selected' : '' }}>{{ __('tasks.due_desc') }}
                </option>
            </select>
        ...
        <div
            ...
            <table class="w-full">
                <thead class="bg-gray-100 text-gray-500 w uppercase tracking-wide text-sm">
                    <tr>
                        <th class="py-3 pl-4 pr-3 text-left font-medium">{{ __('tasks.task_name') }}</th>
