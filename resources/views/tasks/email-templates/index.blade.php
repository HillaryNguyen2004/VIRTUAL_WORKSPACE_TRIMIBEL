...
@section('content')
    ...
    <div class="flex flex-col gap-6 w-full w-max-[1200px] mx-auto text-main px-4 md:px-8 lg:px-16 xl:px-24 py-8">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center w-full mb-8">
            ...
            @if(auth()->user()->hasRole('admin') || auth()->user()->hasDepartmentRolePermission('admin.email_templates.create'))
            ...
            <form class="p-5 border-b border-muted-200 flex flex-wrap gap-4 bg-white" method="GET">
                <div class="flex-1 min-w-[200px] relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="h-5 w-5 text-muted-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                            fill="currentColor">
                    ...
                    <input name="search" id="search" type="text" placeholder="{{ __('template.search_placeholder') }}"
                ...
                <select name="sort_by"
                    ...
                    <option value="created_at" {{ request('sort_by') === 'created_at' ? 'selected' : '' }}>
                        {{ __('template.sort_by_created_at') }}</option>
                    <option value="name" {{ request('sort_by') === 'name' ? 'selected' : '' }}>
                        {{ __('template.sort_by_name') }}</option>
                </select>

                <select name="sort_dir"
                    class="bg-canvas border border-muted-200 text-main py-2.5 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer hover:border-primary/50">
                    <option value="">{{ __('template.default_sort') }}</option>
                    <option value="desc" {{ request('sort_dir') === 'desc' ? 'selected' : '' }}>{{ __('template.desc') }}
                    </option>
                    <option value="asc" {{ request('sort_dir') === 'asc' ? 'selected' : '' }}>{{ __('template.asc') }}
                    </option>
                </select>
            ...
            <div class="overflow-x-auto w-full">
                <table class="w-full table-fixed">
                    <thead class="bg-muted-50 border-b border-muted-200">
                        <tr>
                            <th
                                class="w-[10%] py-4 pl-6 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">
                                ID</th>
                            <th
                                class="w-[30%] py-4 px-3 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">
                                {{ __('template.name') }}</th>
                            <th
                                class="w-[45%] py-4 px-3 text-left text-xs font-semibold text-muted-400 uppercase tracking-wider">
                                {{ __('template.subject') }}</th>
                            <th
                    ...
                    <tbody class="divide-y divide-muted-100">
                        @forelse($templates as $template)
                            <tr class="hover:bg-canvas transition-colors group">
                                ...
                                <td class="py-4 pr-6 text-right">
                                    <div class="flex justify-end gap-1">
                                        {{-- Toggle View --}}
                                        <button
                                            class="toggle-row p-1.5 rounded-lg text-muted-400 hover:bg-primary/5 hover:text-primary transition-colors focus:outline-none"
                                        ...
                                        {{-- Edit --}}
                                        @if(auth()->user()->hasRole('admin') || auth()->user()->hasDepartmentRolePermission('admin.email_templates.edit'))
                                        ...
                                        {{-- Delete --}}
                                        @if(auth()->user()->hasRole('admin') || auth()->user()->hasDepartmentRolePermission('admin.email_templates.delete'))
                            ...
                            {{-- Details Row --}}
                            <tr id="taskDetails{{ $template->id }}"
                                class="hidden bg-canvas border-b border-muted-100 shadow-inner">
                                <td colspan="4" class="p-6">
                                    <div class="flex flex-col gap-6">
                                        <div>
                                            <strong
                                                class="text-sm font-bold text-main">{{ __('template.description') }}</strong>
                                        ...
                                        <div class="bg-white border border-muted-200 rounded-xl p-4">
                                            <strong class="text-sm font-bold text-main block mb-2">{{ __('template.content') }}
                                                preview</strong>
                                            <div
                                                class="text-muted-600 text-sm font-mono bg-muted-50 p-3 rounded-lg max-h-[300px] overflow-auto">
                                                {{-- Using strip_tags to safely preview content or keep {!! !!} if you trust the
                                                source --}}
                                                {!! $template->content !!}
                                            </div>
                                        </div>
