...
@section('title', __('ai.create_workspace'))
...
@section('content')
    <div class="flex flex-col gap-6 w-full max-w-2xl mx-auto text-main px-4 md:px-8 py-8">
        ...
        <div class="w-full bg-white rounded-2xl p-8 border border-muted-200 shadow-lg shadow-main/5 relative overflow-hidden animate-fade-in-up">
            ...
            <form action="{{ route('ai-workspaces.store') }}" method="POST" class="relative z-10">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <label class="relative flex flex-col p-4 rounded-xl border border-muted-200 cursor-pointer hover:bg-muted-50 transition-all group has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:ring-2 has-[:checked]:ring-primary/20">
                                <input type="radio" name="visibility" value="private" class="absolute opacity-0" {{ old('visibility', 'private') === 'private' ? 'checked' : '' }} required>
                            ...
                            {{-- Team --}}
                            <label class="relative flex flex-col p-4 rounded-xl border border-muted-200 cursor-pointer hover:bg-muted-50 transition-all group has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:ring-2 has-[:checked]:ring-primary/20">
                                <input type="radio" name="visibility" value="team" class="absolute opacity-0" {{ old('visibility') === 'team' ? 'checked' : '' }}>
                                <div class="flex items-center gap-3 mb-2">
                            ...
                            {{-- Public --}}
                            <label class="relative flex flex-col p-4 rounded-xl border border-muted-200 cursor-pointer hover:bg-muted-50 transition-all group has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:ring-2 has-[:checked]:ring-primary/20">
                                <input type="radio" name="visibility" value="public" class="absolute opacity-0" {{ old('visibility') === 'public' ? 'checked' : '' }}>
                                <div class="flex items-center gap-3 mb-2">
