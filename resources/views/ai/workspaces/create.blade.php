...
<div id="create-workspace-modal" class="fixed inset-0 z-50 items-center justify-center p-4"
    ...
    <div class="relative w-full max-w-2xl bg-canvas rounded-2xl border border-muted-200 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-fade-in-up">
        ...
        <div class="flex items-start justify-between px-6 py-5 border-b border-muted-200 relative z-10 flex-shrink-0">
            <div>
                <h2 class="font-bold text-xl text-main tracking-tight">{{ __('ai.create_workspace') }}</h2>
                <p class="text-muted-500 text-sm mt-0.5">{{ __('ai.create_workspace_desc') }}</p>
            </div>
        ...
        <div class="overflow-y-auto flex-1">
            <form action="{{ route('ai-workspaces.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-5">
                    <x-form.input label="ai.workspace_name" name="name" :value="old('name')"
                        placeholder="ai.workspace_name_placeholder" :isRequired="true" />

                    <x-form.textarea label="ai.description" name="description" :value="old('description')"
                    ...
                    <div>
                        ...
                        <div class="flex items-stretch w-full text-sm font-medium text-muted-500 bg-white p-1 border border-muted-300 rounded-2xl">
                            ...
                            <div id="create-vis-d1" class="w-px bg-muted-300 my-1 transition-opacity duration-300"></div>
                            ...
                            <div id="create-vis-d2" class="w-px bg-muted-300 my-1 transition-opacity duration-300"></div>
                    ...
                    {{-- Allow others upload --}}
                    <label class="relative block rounded-xl border border-muted-200 bg-canvas p-4 cursor-pointer transition-all hover:bg-white has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:ring-2 has-[:checked]:ring-primary/20">
...
<script>
    (function () {
        ...
        const d1 = document.getElementById('create-vis-d1');
        const d2 = document.getElementById('create-vis-d2');
