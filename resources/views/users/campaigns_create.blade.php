...
@section('content')
    ...
    <x-action-layout :route="'campaigns.index'" :title="'campaigns_create.back_to_campaigns'">
        ...
        <x-admin.campaigns-form :campaign="$campaign ?? null" :users="$users" :templates="$templates" :title="isset($campaign) ? __('campaigns_create.edit_campaign') : __('campaigns_create.create_new_campaign')" />
    </x-action-layout>
            ...
            <div class="card-body">
                <form method="POST"
                    ...
                    <div class="form-check mb-3">
                        ...
                        <label for="send_to_all" class="form-check-label">{{ __('campaigns_create.send_to_all_users')
                            }}</label>
                    </div>
                    ...
                    <div id="user-select-wrapper" class="mb-3" style="{{ old('send_to_all') ? 'display: none;' : '' }}">
                        ...
                        <select name="users[]" class="form-select select2" multiple>
                            ...
                            <option value="{{ $user->id }}" {{ (collect(old('users', isset($campaign) ? $campaign->
                                users->pluck('id')->toArray() : []))->contains($user->id)) ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="email_template_id" class="form-select">
                            ...
                            <option value="{{ $template->id }}" {{ old('email_template_id', $campaign->email_template_id ??
                    ...
                    <button type="submit" class="btn btn-success mt-3">
                        <i class="bi bi-send"></i> {{ isset($campaign) ? __('campaigns_create.update_campaign') :
