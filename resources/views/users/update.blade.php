@props(['users' => collect(), 'user' => null, 'allUsers' => collect()])

@php
    $userId = optional($user)->id;
    $roles = $user ? $user->getRoleNames() : collect();
    $rolePriority = ['admin', 'subadmin', 'staff', 'substaff', 'user'];

    $role = collect($rolePriority)->first(fn($r) => $roles->contains($r)) ?? 'user';
    $teamMembers = $allUsers ? $allUsers->filter(fn($u) => $u->team_leader_id == $userId) : collect();
    $teamOptions = collect($allUsers ?? [])
        ->filter(fn($u) => optional($u->getRoleNames())->first() === 'user' && $u->id !== $userId)
    ...
    $roleOptions = [
        ...
        'subadmin' => __('user_row.subadmin_role'),
        'substaff' => __('user_row.substaff_role'),
    ];
@endphp
...
<div id="update-user-dialog" class="hidden items-center justify-center fixed h-screen w-screen bg-black/50 z-50">
    ...
    <div
        class="flex flex-col w-[320px] sm:w-[450px] lg:w-[500px] bg-white rounded-2xl shadow-xl animate-fade-in-up [animation-delay:150ms] overflow-hidden">
        ...
        <div class="w-full px-6 py-4 flex items-center justify-between border-b border-muted-200 bg-white">
            ...
            <button
                class="close-update-user p-2 rounded-full text-muted-400 hover:text-primary hover:bg-muted-50 transition-colors">
        ...
        <form id="edit-form-{{ $userId }}" action="{{ route('users.update', $userId) }}" method="POST"
            ...
            <div class="p-6 flex flex-col gap-5 w-full">
                ...
                {{-- Name Input --}}
                <x-form.input label="update_user.full_name_label" name="name" id="name" :value="$user->name" />

                {{-- Role Select --}}
                <x-form.select label="update_user.role_label" name="role" id="role" :value="$role" :options="$roleOptions" />

                {{-- Team Members --}}
                <div id="team-select" class="flex flex-col gap-1.5 w-full">
                    <div class="flex items-center justify-between">
                        <label class="text-sm font-medium text-main">{{ __('user_row.assign_team_members') }}</label>
                    </div>

                    <div id="team-members-wrapper"
                        class="flex flex-col gap-3 w-full max-h-48 overflow-y-auto pr-1">

                        @forelse($teamMembers as $member)
                            <div class="team-member-select flex gap-2 w-full items-center">
                                <div class="w-full">
                                    <x-form.select name="team_members[]" placeholder="user_row.select_member"
                                        :value="$member->id" :options="$teamOptions" />
                                </div>
