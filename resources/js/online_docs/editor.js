...
const updateRecentColorSelect = (colors) => {
    ...
    const options = Array.from(recentColorSelect.options).filter((option) => option.value);
        ...
        return {
            ...
            fontFamily: {
                ...
                renderHTML: (attributes) => {
                    ...
                    const family = needsQuotes ? `"${attributes.fontFamily}"` : attributes.fontFamily;
...
const getSelectionFontFamily = (editor) => {
    try {
        ...
        const element = dom.nodeType === Node.TEXT_NODE ? dom.parentElement : dom;
...
const updateToolbarState = (editor) => {
    ...
    if (fontFamilySelect) {
        ...
        updateFontFamilySelect(fontFamilySelect, currentFamily || computedFamily);
    }

    if (lineHeightSelect) {
        const currentLineHeight = editor.getAttributes('heading')?.lineHeight
...
if (editorRoot && form && contentField) {
    ...
    const sanitizeHtml = (html) => DOMPurify.sanitize(html, {
        ADD_ATTR: ['style', 'class', 'colspan', 'rowspan', 'src', 'alt', 'title', 'width', 'height', 'data-table-style'],
        ADD_TAGS: ['table', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td', 'colgroup', 'col', 'mark', 'img']
    });
        ...
        onUpdate: ({ editor: editorInstance }) => {
            ...
            saveTimer = setTimeout(async () => {
                try {
                    ...
                    const sanitized = sanitizeHtml(editorInstance.getHTML() || '');
                    const payload = {
                    ...
                    setStatus(form.dataset.savedText || 'Saved', 'text-emerald-600');
                } catch (error) {
        ...
        toolbar.addEventListener('click', (event) => {
            ...
            switch (command) {
                ...
                case 'findNext':
                    ...
                    {
                        const match = findNextMatch(editor, findInput.value.trim());
                        if (match) {
                ...
                case 'replace':
                    ...
                    if (!findInput.value.trim()) return;
                    {
                        ...
                        if (!empty) {
                            editor.chain().focus().insertContent(replaceInput.value).run();
                        } else {
                            const match = findNextMatch(editor, findInput.value.trim());
                            if (match) {
                                editor.commands.setTextSelection(match);
                ...
                case 'replaceAll':
                    if (!findInput || !replaceInput) return;
                    if (!findInput.value.trim()) return;
                    replaceAllMatches(editor, findInput.value.trim(), replaceInput.value);
                    updateToolbarState(editor);
                    break;
                default:
                    break;
    ...
    if (lineHeightSelect && !isReadOnly) {
        lineHeightSelect.addEventListener('change', (event) => {
            ...
            if (editor.isActive('heading')) {
                editor.chain().focus().updateAttributes('heading', { lineHeight: next }).run();
            } else {
                editor.chain().focus().updateAttributes('paragraph', { lineHeight: next }).run();
            }
            updateToolbarState(editor);
        });
    }
